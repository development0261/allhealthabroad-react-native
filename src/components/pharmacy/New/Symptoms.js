...
export default class Refill_extra extends Component{
    ...
    next_click(){
        ...
        if(this.state.sympotms==""){
            alert("Please describe sympotms");
        }else{
            this.props.navigation.navigate('New_extra',{Json_value:this.state.location_id,mainProvider:this.state.main_provider,provider_id:this.state.provider_id,patient_id:this.state.patient_id,firstName:this.state.first_name,lastName:this.state.last_name,type:this.state.type,providerName:this.state.provider_name,locationName:this.state.location_name,pharmacy_request:this.state.pharmacy_request,subProvider_id:this.state.subProvider_id,sympotms:this.state.sympotms});
        }
    }
    go_back(){
        this.props.navigation.navigate('Select_provider',{Json_value:this.state.location_id,provider_id:this.state.provider_id,patient_id:this.state.patient_id,firstName:this.state.first_name,lastName:this.state.last_name,type:this.state.type,providerName:this.state.provider_name,locationName:this.state.location_name,pharmacy_request:'new'});
    }
    ...
    componentDidMount(){
        ...
        this.setState({location_id:locationId,provider_id:providerId,patient_id:patientId,first_name:FirstName,last_name:LastName,type:type,provider_name:providername,location_name:location,subProvider_id:sub_providerId,provider_type:providertype,main_provider:mainProvider,pharmacy_request:pharmacyRequest});
      ...
      const itemId = navigation.getParam('Json_value', 'NO-ID');
        ...
        <View style={styles.container}>
            ...
            <View>
                ...
                <Text style={styles.sub_title}>Step 5 of 7</Text>
            </View>
            <ScrollView>
                <View style={styles.first_contanier}>
                    <HideWithKeyboard><Text style={{textAlign:'center',fontSize:20,color:'#0097a7',marginTop:10,marginBottom:10}}>What are your symptoms?</Text></HideWithKeyboard>
                    ...
                    <View style={{marginTop:20,width:'100%'}}>
                        <TextInput
                            ...
                            multiline={true}
                            numberOfLines={10}
                            onChangeText={(sympotms) => this.setState({sympotms})}
                        />
                    </View>
                </View>
            </ScrollView>
